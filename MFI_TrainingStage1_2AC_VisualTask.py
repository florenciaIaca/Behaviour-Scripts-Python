# 2AC Visual task
# Trail starts with a LED, that is ON until the correct port is licked and then there is a reward and the LED goes off until the next trial
...
LEDsIDs[0 : (maxRews/2)]=1
LEDsIDs[(maxRews/2) : maxRews]=2
rnd.shuffle(LEDsIDs)
print LEDsIDs
   ...
   # Check to make sure that only 1 lick is detected if the mouse makes contact.
        # Corrects for switch bounces from the relay in the lick
            ...
            # Play the sound, needs to change to present 1 of the 2 sounds
                ...
                if LEDsIDs[nRews]== 1:
                    GPIO.output(LEDPINL,1)  #set pin to high=3.3V
##                    print 'Sound = 8KHz, (R)'
