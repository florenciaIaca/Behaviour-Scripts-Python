...
# 2AC Visual task
# Training stage 2: Same as stage 0 but without a free reward at the start of every trial. Reward is only given if the animal licks the correct spout.
# The stimulus lasts until the mouse licks the correct spout and collects the reward. Mice are expected to achieve 1 block of 50 trails for each stimulus.
# A free reward is given on the correct side if the animal licks the incorrect side more than 4 times.
# Stimulus duration is set to 100 ms and then the stimulus disappears.
# If the grating flickers try replacing while for if
# Author: Maria Florencia Iacaruso
# contact: florencia.iacaruso@gmail.com

print 'Running Training stage 2 Visual Task'
...
# Corrects for switch bounces from the relay in the lick
    ...
    # Play the grating
        ...
        if sideIDs[nRews]== 1:
            ...
            if (soundCounter==0) and (trialClock.getTime() > stimDur): 
                tock.play()
                soundCounter=1;
