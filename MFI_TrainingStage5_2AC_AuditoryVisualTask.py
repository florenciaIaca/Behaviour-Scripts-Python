# 2AC Auditory and visual task training stage 5
# Trail starts with a sound/LED light that is presented for 100 ms. If the correct spout is licked there is a reward,
# otherwise a punishement of 20 seconds out is introduced. The animal will have to wait 3 seconds to start licking to get a reward.
# Otherwise the trial will start again.
# Visual and auditory stimuli will be presented simultaneously. Only auditory will be rewarded
# Author: Maria Florencia Iacaruso
# contact: florencia.iacaruso@gmail.com

print 'Running Training stage 5 Sound and visual as a distractor'
...
Training = True
stimDur=0.100; # duration of visual or auditory stimuli
nRews = 0
maxRews = 12 ; # number of rewards for visual or auditory blocks should be set to 300
nRewSpecific=10 # Number of initial rewards without distractor
...
minILI = 0.05 # used to be 0.01
minILIPunish = 3 # If the animal licks before minILIPunish there is a time out of 3 secons.
punishDur=20;# Time in seconds of punishment if the wrong spout is licked

##plt.hist(randDelays)
##fig=plt.gcf
##plt.show()

# Generate the stimulus sequence. There are 6 options in the Auditory task. VL&SL, LR&SR, VL&SR, VR&SL, SL-, SR-
# The task will start with 10 trials of visual only. To give the animal the initial indication that it is a auditory task.

StimIDs3=np.zeros(0)
for iterNr in range(nBlocks):
    # Generate array of random order for presentation of sounds
    SoundIDs=np.zeros(6)
    SoundIDs[0 : 6]=np.arange(1.,7.)
    SoundIDs=SoundIDs.repeat(maxRews/np.size(SoundIDs))
#    SoundIDs=np.tile(SoundIDs,[1,maxRews/np.size(SoundIDs)])
    rnd.shuffle(SoundIDs)

    # Generate array of random order for presentation of sounds
    SoundIDs2=np.zeros(2)
    SoundIDs2[0 : 2]=np.arange(1.,3.)
    SoundIDs2=SoundIDs2.repeat(nRewSpecific/2)
    rnd.shuffle(SoundIDs2)
    
    StimIDs3=np.concatenate((SoundIDs2, SoundIDs),axis=1)
    
print StimIDs3

maxRews3=len(StimIDs3);
   ...
   # Check to make sure that only 1 lick is detected if the mouse makes contact.
        # Corrects for switch bounces from the relay in the lick
            ...
            # Play the sound, needs to change to present 1 of the 2 sounds
                ...
                if StimIDs3[nRews]== 1: # Auditory task only lick left
                    ...
                    if(GPIO.event_detected(lickL)):
                        if GPIO.input(lickL):
							print 'L'
		                    if (time.time()-prevL)>minILIPunish:
		                        lickT = time.time()
                    ...
                    if StimIDs3[nRews]== 2: # Auditory task only lick Right
                        if nRews == len(stimList2):
                        ...
                        if StimIDs3[nRews]== 3: # Sound Left, LED left Lick Left
                            
                            if nRews == len(stimList2):
                                stimList.append([time.time()-start,'Sound8KHz-LEDL'])
                                stimList2.append([time.time()-start,'Sound8KHz-LEDL'])
                                sndArray1.play()
                            ...
                            if StimIDs3[nRews]== 4: # Sound Right, LED Right Lick Right
                                
                                if nRews == len(stimList2):
                                    stimList.append([time.time()-start,'Sound15KHz-LEDR'])
                                    stimList2.append([time.time()-start,'Sound15KHz-LEDR'])
                                    sndArray2.play()
                                ...
                                if StimIDs3[nRews]== 5: # Sound Left, LED left Lick Left
                                    
                                    if nRews == len(stimList2):
                                        stimList.append([time.time()-start,'Sound8KHz-LEDR'])
                                        stimList2.append([time.time()-start,'Sound8KHz-LEDR'])
                                        sndArray1.play()
                                        GPIO.output(LEDPINR,1)
                                        time.sleep(stimDur)
                                        pygame.mixer.stop()
                                    ...
                                    if StimIDs3[nRews]== 6: # Sound Right, LED Right Lick Right
                                        
                                        if nRews == len(stimList2):
                                            stimList.append([time.time()-start,'Sound15KHz-LEDL'])
					    					stimList2.append([time.time()-start,'Sound15KHz-LEDL'])
                                            sndArray2.play()
                                            GPIO.output(LEDPINL,1)
                                            time.sleep(stimDur)
                                            pygame.mixer.stop()
                ...
                if (nRews>(maxRews3-1)) or time.time() - start > ExpDur:
                    ...
                    print ['Total trials = ' , len(rewList)]
                    lickList, rewList, stimList, sendT = data_sender(lickList,rewList,stimList,sendT)
